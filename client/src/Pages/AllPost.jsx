...
export default function AllPost() {
  ...
  useEffect(  ()=>{
    ...
    try {
        setLoading(true)
        const data=await axios.post('get-all-blog',{},{withCredentials:true})
        if(!data){
            setLoading(false)
...
try {
  ...
  return (
    <div className="flex flex-col w-full font-serif items-center p-6 bg-black text-white min-h-screen">
      ...
      <h1 className="text-2xl font-bold border-b-2 border-white pb-2 mb-6">My Blog</h1>


      {
        loading?(<LoaderCircle className="text-white animate-spin " />):(<ul className="w-full max-w-3xl space-y-4"> {/* wider container */}
